...
class allinone::boton_cerrar_sesion {
    exec { "copia_xfce-session-logout":
        command => "cp /usr/bin/xfce4-session-logout /usr/bin/xfce4-session-logout.orig",
        path => ['/usr/bin', '/bin', '/usr/sbin', '/sbin'],
        unless => "test -f /usr/bin/xfce4-session-logout.orig",
    ...
    exec { "instala_salida_automatica":
        ...
        path => ['/usr/bin', '/bin', '/usr/sbin', '/sbin'],
        onlyif => "test -f /usr/bin/xfce4-session-logout.orig",
        unless => "grep -- '--logout' /usr/bin/xfce4-session-logout",
        notify => Exec["cambia_icono_logout"]
    }

    exec { "cambia_icono_logout":
        command => "sed -i 's/+logout/-logout/' /etc/xdg/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml",
        path => ['/usr/bin', '/bin', '/usr/sbin', '/sbin'],
        unless => "grep '\"-logout\"' /etc/xdg/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml",
        notify => Exec["nuevo_icono_logout-dialog"]
    }

    exec { "nuevo_icono_logout-dialog":
        command => "sed -i 's/-logout-dialog/+logout-dialog/' /etc/xdg/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml",
        path => ['/usr/bin', '/bin', '/usr/sbin', '/sbin'],
        unless => "grep '\"+logout-dialog' /etc/xdg/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml",
    }
}
