...
class siatic::hostname {
    package{"python3-ldap3":
        ensure => "installed"
    }

    file {"/root/autorename.py":
        source => "puppet:///modules/siatic/autorename.py",
        owner => root,
        group => root,
        mode => '644',
    ...
    exec { "/usr/bin/python3 /root/autorename.py":
        require => File["/root/autorename.py"]
    }
}
